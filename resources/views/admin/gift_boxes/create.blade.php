        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('admin.gift-boxes.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-6">
                            ...
                            <select id="flower_id" name="flower_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach ($flowers as $flower)
                                    <option value="{{ $flower->product_id }}">{{ $flower->name }} - ${{ number_format($flower->price, 2) }}</option>
                                @endforeach
                            </select>
                            @error('flower_id')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        ...
                        <div class="mb-6">
                            ...
                            <select id="chocolate_id" name="chocolate_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach ($chocolates as $chocolate)
                                    <option value="{{ $chocolate->product_id }}">{{ $chocolate->name }} - ${{ number_format($chocolate->price, 2) }}</option>
                                @endforeach
                            </select>
                            @error('chocolate_id')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        ...
                        <div class="mb-6">
                            ...
                            <select id="packaging_id" name="packaging_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                ...
                                @foreach ($packagings as $packaging)
                                    <option value="{{ $packaging->product_id }}">{{ $packaging->name }} - ${{ number_format($packaging->price, 2) }}</option>
                                @endforeach
                            </select>
                            @error('packaging_id')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        ...
                        <div class="mb-6">
                            ...
                            <input type="number" name="quantity" id="quantity" value="{{ old('quantity', 1) }}" min="1" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                            @error('quantity')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <input type="number" name="price" id="price" value="{{ old('price') }}" step="0.01" min="0" class="pl-7 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                            </div>
                            @error('price')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="custom_message" id="custom_message" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('custom_message') }}</textarea>
