    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between">
                <h1 class="text-2xl font-semibold text-gray-900">Edit Promotion</h1>
            ...
            <div class="mt-6 bg-white shadow overflow-hidden rounded-lg">
                <form action="{{ route('admin.promotions.update', $promotion->promotion_id) }}" method="POST" class="p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative rounded-md shadow-sm">
                                ...
                                <input type="number" name="minimum_order" id="minimum_order" value="{{ old('minimum_order', $promotion->minimum_order) }}" step="0.01" min="0"
                                       class="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-7 sm:text-sm border-gray-300 rounded-md">
                            </div>
                            @error('minimum_order')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">{{ old('description', $promotion->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mt-6">
                        <label class="block text-sm font-medium text-gray-700">Current Usage Count</label>
                        <div class="mt-1 px-3 py-2 bg-gray-100 rounded-md text-gray-700">
